      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          ...
          <form onSubmit={handleApplyFilter} className="space-y-4">
              ...
              <div className="md:col-span-5 relative">
                ...
                <div className="relative">
                  ...
                  <input
                    ...
                    className="w-full pl-10 rtl:pl-3 rtl:pr-10 pr-3 py-2 border border-[#E2E8F0] rounded-xl text-sm text-[#1E293B] focus:border-[#2563EB] focus:outline-none bg-[#FAF9F6] focus:bg-white transition-colors font-medium"
                  />
                </div>
              </div>
            ...
            <div className="flex flex-wrap items-center justify-between gap-3 pt-3 border-t border-[#F1F5F9]">
              <div className="flex items-center gap-1.5">
                <span className="text-xs font-semibold text-[#64748B] mr-1 rtl:mr-0 rtl:ml-1">{t('search.category')}</span>
        ...
        {isLoading ? (
          <div className="py-20 text-center">
            ...
            <p className="text-sm font-medium text-[#64748B]">{t('search.searching')}</p>
          </div>
        ) : totalCount === 0 ? (
          <div className="bg-white p-12 rounded-2xl border border-[#E2E8F0] text-center max-w-lg mx-auto shadow-xs">
            ...
            <h3 className="text-lg font-bold text-[#1E293B] mb-1">{t('search.noResultsTitle')}</h3>
                    ...
                    >
                      ...
                      <div className="mt-4 pt-3 border-t border-[#F1F5F9] flex items-center justify-between text-xs">
                        ...
                        <div className="flex items-center gap-2">
                          <Link
                            to={`/doctors/${doc.id}`}
                            className="px-3 py-1.5 font-semibold text-[#475569] hover:bg-[#FAF9F6] rounded-lg border border-[#E2E8F0] cursor-pointer"
                          >
                            {t('search.details')}
                          </Link>
                          <Link
                            to={`/book/doctor/${doc.id}`}
                            className="px-3.5 py-1.5 font-bold bg-[#2563EB] hover:bg-blue-700 text-white rounded-lg shadow-sm cursor-pointer"
                          >
                            {t('search.bookSlot')}
                          </Link>
                        </div>
                  ...
                  {filteredHospitals.map((hosp) => (
                    ...
                    >
                      <div>
                        <div className="relative aspect-16/9 bg-slate-100">
                          ...
                          {hosp.emergencyAvailable && (
                            <span className="absolute top-2.5 left-2.5 rtl:left-auto rtl:right-2.5 bg-rose-600/90 backdrop-blur-xs text-white text-[10px] font-bold px-2 py-0.5 rounded-md shadow-xs">
                              {t('search.emergency247')}
                            </span>
                          )}
                          <div className="absolute top-2.5 right-2.5 rtl:right-auto rtl:left-2.5 bg-white/95 backdrop-blur-xs px-2 py-0.5 rounded-md text-xs font-bold text-[#1E293B] flex items-center gap-1 shadow-xs">
                            <Star className="w-3.5 h-3.5 text-amber-500 fill-amber-500" />
                        ...
                        <div className="p-4">
                          ...
                          <p className="text-xs text-[#64748B] mb-2">{translateLocation(hosp.location)}</p>
                      ...
                      <div className="p-4 pt-0 flex items-center justify-between border-t border-[#E5E7EB] pt-3 text-xs">
                        <span className="text-[#64748B] font-semibold">{t('search.specialistsCount', { count: hosp.doctorCount })}</span>
                        <Link
                          to={`/hospitals/${hosp.id}`}
                          className="px-3.5 py-1.5 font-bold text-[#2563EB] hover:bg-blue-50 rounded-lg flex items-center gap-1 cursor-pointer"
                        >
                  ...
                  {filteredClinics.map((clinic) => (
                    ...
                    >
                      ...
                      <div className="mt-4 pt-3 border-t border-[#E5E7EB] flex items-center justify-between text-xs">
                        <span className="text-[#64748B] font-semibold">{t('search.doctorsCount', { count: clinic.doctorCount })}</span>
                        <Link

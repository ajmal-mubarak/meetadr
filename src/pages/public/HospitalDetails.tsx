        ...
        <div className="bg-white rounded-2xl border border-slate-200 overflow-hidden shadow-xs mb-8">
          <div className="relative h-64 sm:h-80 w-full bg-slate-200">
            ...
            {hospital.emergencyAvailable && (
              <span className="absolute top-4 left-4 bg-rose-600 text-white text-xs font-bold px-3 py-1 rounded-lg shadow-md">
                24/7 Emergency Services
              </span>
            )}
            <div className="absolute top-4 right-4 bg-white/95 backdrop-blur-xs px-3 py-1 rounded-xl text-xs font-bold text-slate-900 flex items-center gap-1 shadow-md">
          ...
          <div className="flex items-center justify-between mb-6">
            <div>
              ...
              <p className="text-xs text-slate-500 mt-0.5">
                Select a specialist to view availability and book an appointment slot.
              </p>
            </div>
            ...
            {doctors.map((doc) => (
              ...
              >
                <div>
                  <div className="flex items-start gap-4">
                    ...
                    <div className="min-w-0 flex-1">
                      <span className="text-xs font-semibold text-blue-600">{doc.specialty}</span>
                      <h3 className="text-base font-bold text-slate-900 leading-snug truncate">
                        {doc.name}
                      </h3>
                      <p className="text-xs text-slate-500 truncate">{doc.experience} Experience</p>
                ...
                <div className="mt-4 pt-3 border-t border-slate-100 flex items-center justify-between gap-2">
                  <Link
                    to={`/doctors/${doc.id}`}
                    className="flex-1 py-2 text-center text-xs font-semibold text-slate-700 bg-slate-50 hover:bg-slate-100 rounded-lg border border-slate-200"
                  >
                    Details
                  </Link>
                  <Link
                    to={`/book/doctor/${doc.id}`}
                    className="flex-1 py-2 text-center text-xs font-semibold text-white bg-blue-600 hover:bg-blue-700 rounded-lg shadow-xs"
                  >
                    Book
                  </Link>
                </div>
